...
interface WishlistDisplayProps {
  content?: string;
  contentHtml?: string; // from API (already with auto-linked URLs)
  personName: string;
}

/**
 * Component to display the wishlist read-only
...
function WishlistDisplay({ content, contentHtml, personName }: WishlistDisplayProps) {
  ...
  // If API already returned HTML, we use it; otherwise we convert the content
  const htmlContent = contentHtml || (content ? convertToHtml(content) : "");

  // If there is no content, we show the empty state
  if (!htmlContent) {
    return (
      <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border-2 border-red-200 dark:border-red-700 p-6 relative overflow-hidden">
        {/* Christmas decorations in the background */}
        ...
        <div className="relative z-10">
          ...
          {/* Empty state with Christmas theme */}
