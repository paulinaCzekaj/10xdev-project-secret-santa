...
describe("NotificationService", () => {
  ...
  describe("success()", () => {
    ...
    it("should display success notification with custom message", () => {
      ...
      expect(toast.success).toHaveBeenCalledWith("Custom", expect.objectContaining({ description: "Desc" }));
    });

    it("should pass options to toast", () => {
      notify.success("AUTH.LOGIN_SUCCESS", { duration: 5000 });

      expect(toast.success).toHaveBeenCalledWith("Zalogowano pomyślnie!", expect.objectContaining({ duration: 5000 }));
    });
  });
  ...
  describe("error()", () => {
    ...
    it("should display error notification with custom message", () => {
      ...
      expect(toast.error).toHaveBeenCalledWith("API Error", expect.objectContaining({ description: "Details" }));
    });
  });
  ...
  describe("getMessage()", () => {
    ...
    it("should return undefined for invalid key", () => {
      const message = notify.getMessage("INVALID.KEY");
  ...
  describe("Unknown message key handling", () => {
    it("should use unknown key as title with warning", () => {
      ...
      expect(consoleSpy).toHaveBeenCalledWith(expect.stringContaining("Unknown message key"));
      expect(toast.success).toHaveBeenCalledWith("UNKNOWN.KEY", expect.any(Object));
